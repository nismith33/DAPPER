...
def plt_rmse_sig_field(xps, axes, names):
    """Calculate time-averaged RMSE."""
    n_cells = 16.

    # Calcalute obs density
    sigs, rmse_for, rmse_ana = [], {}, {}
    for xp in xps[:7]:
        db = xp.obs_db
        sigs.append(np.sqrt(np.mean(xp.obs_db['var'])))
        print('calculate ', sigs[-1])
    ...
    for ax, name in zip(axes, names):
        ...
        ax.plot(np.array(sigs)*1e-2, rmse_for[name], 'k-',
                alpha=0.3, linewidth=4.,
                label='Forecast')
        ax.plot(np.array(sigs)*1e-2, rmse_ana[name], 'k-',
        ...
        ax.set_xlabel('Obs. error std. dev. stress [hPa]', fontsize=font['size'])


def plt_rmse_sigs(save_fig=False):
    ...
    plt_rmse_sig_field(xps, axes, names)

    if save_fig:
        fig_path = os.path.join(fig_dir, 'rmse_sigs.png')
