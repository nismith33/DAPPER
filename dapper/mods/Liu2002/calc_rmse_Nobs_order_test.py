...
# Directory
fig_dir = os.path.join(exp.fig_dir,'rmse_Nobs_order_test')
...
def exp_spectral_Nobs(T=3, Nens=3, sig_obs=1.0):
...
def test(xps):
    r = np.linspace(0, 8e6, exp.Ncells * 100, endpoint=False)
    t = np.size(xps[0].E['ana'],0) - 1
    
    #truth
    xx0  = xps[0].true_HMM.model.functions[0](r,t)
    xx0 += xps[0].true_HMM.noise.functions[0](r,t)
    print('x0 ',np.min(xx0),np.max(xx0),np.mean(xx0**2))
    
    #lin
    xxL = xps[0].E['for'][-1,0]
    xxL = xps[0].HMM.model.interpolator(xxL)(r)[0]
    print('error lin ',np.mean((xxL-xx0)**2))
    
    for n,xp in enumerate(xps[1:]):
        x = xp.E['for'][-1,0]
        x = xp.HMM.model.interpolator(x)(r)[0]
        print('error DG{:d} '.format(xp.parent._order),np.mean((x-xx0)**2))
