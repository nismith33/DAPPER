...
# Directory
fig_dir = os.path.join(exp.fig_dir,'rmse_Nobs_order_sigoinflation')
...
def exp_spectral_Nobs(T=50, Nens=16):
    ...
    for sig in np.arange(.9,1.45,.05):
        Nobs_list = [int(exp.Ncells*o) for o in [1,3,9]]
        for Nobs in Nobs_list:
            tree.add_obs(Nobs, 1, sigo_inflation=sig, obs_type='uniform',
