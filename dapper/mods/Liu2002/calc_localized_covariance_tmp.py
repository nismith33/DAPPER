...
# Directory
fig_dir = os.path.join(exp.fig_dir, 'localized_covariance1')
...
def exp_spectral_Nobs(T=1, Nens=10000, sig_obs=1.0):
    ...
    # Background
    tree = exp.MeanTreeNode(None, tseq, io=io, K=197, sig=0.)
    ...
    for sig in np.array([1.]):
        for Nobs in np.arange(1, 2, 2) * exp.Ncells:  # 8
    ...
    for order in np.array([4]):  # 8
        ...
        tree.add_model('dg', exp.Ncells, order=order,
                       name='DG{:02d}'.format(order), localization=localization)
