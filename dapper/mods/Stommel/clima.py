...
model.ep_flux = [stommel.add_melt(default_ep_flux, model)]
default_temp_air = model.temp_air[0]
model.temp_air = [stommel.add_warming(default_temp_air)]
# Initial conditions
x0 = model.x0
B = np.zeros_like(x0)
...
fig.savefig(os.path.join(fig_dir, 'clima.png'),format='png',dpi=500)
